...
static rg_gui_event_t toggle_tab_cb(rg_gui_option_t *option, rg_gui_event_t event)
{
    ...
    strcpy(option->value, tab->enabled ? TEXT_Show : TEXT_Hide);
...
static rg_gui_event_t toggle_tabs_cb(rg_gui_option_t *option, rg_gui_event_t event)
{
    if (event == RG_DIALOG_ENTER)
    {
        ...
        *opt++ = (rg_gui_option_t)RG_DIALOG_END;
        ...
        rg_gui_dialog(TEXT_Tabs_Visibility, options, 0);
    }
    return RG_DIALOG_VOID;
}
...
{
    const char *modes[SCROLL_MODE_COUNT] = {TEXT_Center, TEXT_Paging};
...
{
    const char *modes[START_SCREEN_COUNT] = {TEXT_Auto, TEXT_Carousel, TEXT_Browser};
...
{
    const char *modes[] = {TEXT_None, TEXT_Cover_Save, TEXT_Save_Cover, TEXT_Cover_only, TEXT_Save_only};
...
{
    const char *modes[] = {TEXT_Last_game, TEXT_Launcher};
    int max = 1;
...
static rg_gui_event_t webui_switch_cb(rg_gui_option_t *option, rg_gui_event_t event)
{
    ...
    strcpy(option->value, enabled ? TEXT_On : TEXT_Off);
    return RG_DIALOG_VOID;
}
#endif
...
static rg_gui_event_t launcher_options_cb(rg_gui_option_t *option, rg_gui_event_t event)
{
    if (event == RG_DIALOG_ENTER)
    {
        const rg_gui_option_t options[] = {
            {0, TEXT_Color_theme, "-", RG_DIALOG_FLAG_NORMAL, &color_theme_cb},
            {0, TEXT_Preview, "-", RG_DIALOG_FLAG_NORMAL, &show_preview_cb},
            {0, TEXT_Scroll_mode, "-", RG_DIALOG_FLAG_NORMAL, &scroll_mode_cb},
            {0, TEXT_Start_screen, "-", RG_DIALOG_FLAG_NORMAL, &start_screen_cb},
            {0, TEXT_Hide_tabs, "-", RG_DIALOG_FLAG_NORMAL, &toggle_tabs_cb},
            #ifdef RG_ENABLE_NETWORKING
            {0, TEXT_File_server, "-", RG_DIALOG_FLAG_NORMAL, &webui_switch_cb},
            #endif
            {0, TEXT_Startup_app, "-", RG_DIALOG_FLAG_NORMAL, &startup_app_cb},
            RG_DIALOG_END,
        };
        gui_redraw(); // clear main menu
        rg_gui_dialog(TEXT_Launcher_Options, options, 0);
    }
    return RG_DIALOG_VOID;
}
...
{
    const rg_gui_option_t options[] = {
        {0, TEXT_Build_CRC_cache, NULL, RG_DIALOG_FLAG_NORMAL, &prebuild_cache_cb},
    #ifdef RG_ENABLE_NETWORKING
        {0, TEXT_Check_for_updates, NULL, RG_DIALOG_FLAG_NORMAL, &updater_cb},
    #endif
        RG_DIALOG_END,
    };
...
static void retro_loop(void)
{
    ...
    while (true)
    {
        ...
        if (gui.http_lock)
        {
            rg_gui_draw_message(TEXT_HTTP_Server_Busy);
            redraw_pending = true;
            rg_task_delay(100);
            continue;
        }
...
void app_main(void)
{
    ...
    const rg_gui_option_t options[] = {
        {0, TEXT_Launcher_Options, NULL, RG_DIALOG_FLAG_NORMAL, &launcher_options_cb},
        RG_DIALOG_END,
    };
    ...
    if (!rg_storage_ready())
    {
        ...
        rg_gui_alert(TEXT_HTTP_SD_Card_Error, TEXT_HTTP_Storage_mount_failed);
    }
    else
    {
